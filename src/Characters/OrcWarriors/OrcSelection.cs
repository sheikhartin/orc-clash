...
namespace OrcClash.OrcWarriors;
...
public partial class OrcSelection : Node2D {
    ...
    public void SpawnSelectedOrc() {
        if (!OrcScenePaths.TryGetValue(Type, out var scenePath)) {
        ...
        GetParent().CallDeferred("add_child", orcInstance);
        orcInstance.GlobalPosition = GlobalPosition;
